...
namespace SDTK{
	...
	public class Geometry {
		...
		//折线偏移
		public static Vector3[] GetOffsetShape(Vector3[] node, float width, bool isClose){
			Vector3[] rt;
			int n=isClose?node.Length: node.Length-1;
			int i=isClose?0:1;
			
			if(n < 3){
				Debug.LogError("Illegal Array");
				return null;
			}
			...
			for(; i<n; i++){
				rt[i]=GetOffsetPoint(node[i], node[(i+1)%n],node[(i-1+n)%n],width);
			}
			...
			rt[node.Length-1]=node[n]+GetPerpendicular(node[n]-node[n-1]).normalized*width;
		...
		public static Vector3 GetOffsetPoint(Vector3 org, Vector3 p1, Vector3 p2, float width){
			...
			if(Measure.PointOfSide(p2, org, p1)>-0.00001)// as >=0, 防止浮点精度导致的错误
				return org+nor*dis;
			else
				return org+nor*-dis;
		}
		private static float IntersectionDis(Vector3 nor,Vector3 dir, float width){
